...
def foo(veugel):
    ...
    for day in veugel.days:
        foodict[to_vijfvoud(day.day)] = [dp.duration_of_state for dp in day.datapoints]
        #for dp in day.datapoints:
        #    vijfvoud = to_vijfvoud(day.day)
        #    foodict[vijfvoud].append(dp.duration_of_state)
...
if __name__ == '__main__':
    for veugel_number in list(relational.BROTHERS):
        ...
        ax0.set_title('{veugel.name} DAS at 50DPH'.format(**locals()))

        ax2.hist(data_60, n_bins, histtype='step', fill=True)
        ax2.set_title('{veugel.name} DAS at 60DPH'.format(**locals()))

        ax4.hist(data_70, n_bins, histtype='step', fill=True)
        ax4.set_title('{veugel.name} DAS at 70DPH'.format(**locals()))

        ax6.hist(data_90, n_bins, histtype='step', fill=True)
        ax6.set_title('{veugel.name} DAS at 90DPH'.format(**locals()))

        ax8.hist(data_120, n_bins, histtype='step', fill=True)
        ax8.set_title('{veugel.name} DAS at 120DPH'.format(**locals()))

        data_50, data_60, data_70, data_90, data_120 = get_hist_data(brother)


        ax1.hist(data_50, n_bins, histtype='step', fill=True)
        ax1.set_title('{brother.name} DAS at 50DPH'.format(**locals()))

        ax3.hist(data_60, n_bins, histtype='step', fill=True)
        ax3.set_title('{brother.name} DAS at 60DPH'.format(**locals()))

        ax5.hist(data_70, n_bins, histtype='step', fill=True)
        ax5.set_title('{brother.name} DAS at 70DPH'.format(**locals()))

        ax7.hist(data_90, n_bins, histtype='step', fill=True)
        ax7.set_title('{brother.name} DAS at 90DPH'.format(**locals()))

        ax9.hist(data_120, n_bins, histtype='step', fill=True)
        ax9.set_title('{brother.name} DAS at 120DPH'.format(**locals()))


        fig.suptitle("{veugel.name} / {brother.name}".format(**locals()))
        fig.savefig("DAS_hist_{veugel.name}_{brother.name}.png".format(**locals()))