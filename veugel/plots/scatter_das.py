...
def plot_scatter_das(fig, veugel, color='red'):
    """
    :type veugel: Veugel
    :param min: int
    :param max: int
...
if __name__ == '__main__':
    for veugel_number in list(relational.BROTHERS):
        ...
        plot_scatter_das(ax1, veugel, color='red')
        plot_scatter_das(ax2, brother, color='blue')

        fig.suptitle("{veugel.name} / {brother.name}".format(**locals()))
        fig.savefig("dasscatter_{veugel.name}_{brother.name}.png".format(**locals()),bbox_inches='tight')
